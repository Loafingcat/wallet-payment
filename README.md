...
- Live Demo: Streamlit Cloud 배포 후 이곳에 URL을 추가하면 됩니다.
...
앱은 사이드바의 4개 메뉴로 구성됩니다.

| 메뉴 | 용도 |
|---|---|
| 시작하기 | 서비스 소개, 데모 데이터 생성, 현재 상태 요약, 5단계 흐름을 보여줍니다. |
| 지갑 사용해보기 | 충전, 결제, 환불, 송금을 실제 서비스처럼 실행합니다. |
| 거래내역과 정산 | 원장 거래내역, 필터, 차트, 가맹점 정산 계산을 확인합니다. |
| 프로젝트 구조 | 정합성, 중복 요청 방지, 동시성 처리, 테스트와 검증을 짧게 설명합니다. |

### 1분 데모 순서

처음 보는 사람에게 보여줄 때는 아래 순서가 가장 이해하기 쉽습니다.

1. `시작하기`에서 `데모 데이터 만들기`를 누릅니다.
2. `지갑 사용해보기`에서 `충전`, `결제`, `환불`을 차례로 눌러봅니다.
3. 결제 탭에서 같은 요청 키로 한 번 더 결제해 중복 요청 방지를 확인합니다.
4. `거래내역과 정산`에서 거래 테이블과 차트를 확인합니다.
5. 같은 화면에서 가맹점을 선택하고 `정산 계산하기`를 누릅니다.

### 직접 눌러볼 기능

| 기능 | 어디서 실행 | 확인할 것 |
|---|---|---|
| 충전 | `지갑 사용해보기` → `충전` | 사용자 잔액 증가, `CHARGE` 거래 추가 |
| 결제 | `지갑 사용해보기` → `결제` | 사용자 잔액 감소, 가맹점 금액 증가, `PAYMENT` 거래 추가 |
| 환불 | `지갑 사용해보기` → `환불` | 기존 결제를 수정하지 않고 `REFUND` 거래 추가 |
| 송금 | `지갑 사용해보기` → `송금` | `TRANSFER_OUT`, `TRANSFER_IN` 쌍으로 기록 |
| 정산 | `거래내역과 정산` | 결제/환불/데모 수수료율 기준 최종 정산 금액 계산 |
